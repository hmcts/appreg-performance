...
/** One-user proof for the simple Application List update action, using an ordinary open list. */
public class UpdateApplicationListProofSimulation extends Simulation {
  private static final String SEEDED_LIST_ID = requiredEnvironmentVariable("APPREG_SEED_UPDATE_LIST_ID");
  private final Iterator<Map<String, Object>> ssoUserFeeder = SsoAuthentication.users(1);

  public UpdateApplicationListProofSimulation() {
    ...
    var updateApplicationList = scenario("AppReg application list update proof")
      .exitBlockOnFail().on(
        feed(ssoUserFeeder)
    ...
    setUp(updateApplicationList.injectOpen(atOnceUsers(1)))
  ...
  private static String requiredEnvironmentVariable(String name) {
    ...
    if (value == null || value.isBlank()) {
      throw new IllegalArgumentException("Set " + name + " to run the seeded application-list-update proof");
    }
    return value;
  }
}
